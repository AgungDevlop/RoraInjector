      ...
      try {
        ...
        recallsData.forEach((recall: RecallData) => {
          ['img1', 'img2'].forEach((imgType) => {
            const img = new Image();
            img.src = recall[imgType as keyof RecallData];
            img.onload = () => {
          ...
          >
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <img
                ...
                className={`w-8 sm:w-10 h-8 sm:h-10 object-cover rounded-full border-2 border-blue-400 animate-neon-pulse ${loadedImages.has(`${recall.id}-img1`) ? '' : 'hidden'}`}
                loading="lazy"
              />
              <FaArrowRight className="text-blue-300 mx-1 text-sm sm:text-base animate-neon-pulse" />
              ...
              <img
                ...
                className={`w-8 sm:w-10 h-8 sm:h-10 object-cover rounded-full border-2 border-blue-400 animate-neon-pulse ${loadedImages.has(`${recall.id}-img2`) ? '' : 'hidden'}`}
                loading="lazy"
              />
