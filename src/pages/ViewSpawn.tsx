  ...
  useEffect(() => {
    const fetchSpawns = async () => {
      try {
        ...
        spawnsData.forEach((spawn: SpawnData) => {
          ['img1', 'img2'].forEach((imgType) => {
            const img = new Image();
            img.src = spawn[imgType as keyof SpawnData];
            img.onload = () => {
              setLoadedImages((prev) => new Set(prev).add(`${spawn.id}-${imgType}`));
            ...
            >
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                <img
                  src={spawn.img1}
                  alt={`${spawn.name} img1`}
                  className={`w-8 sm:w-10 h-8 sm:h-10 object-cover rounded-full border-2 border-blue-400 animate-neon-pulse ${loadedImages.has(`${spawn.id}-img1`) ? '' : 'hidden'}`}
                  loading="lazy"
                />
                ...
                <img
                  src={spawn.img2}
                  alt={`${spawn.name} img2`}
                  className={`w-8 sm:w-10 h-8 sm:h-10 object-cover rounded-full border-2 border-blue-400 animate-neon-pulse ${loadedImages.has(`${spawn.id}-img2`) ? '' : 'hidden'}`}
                  loading="lazy"
                />
